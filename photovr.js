...
var instructions = hub().addTo(env.ui).named("instructions");

// The hook on which we teleport the user back to center.
var home = box(0.1, 0.1, 0.1).textured("images/home.png", {
  transparent: true,
  unshaded: true
}).named("home").addTo(env.ui).at(0, 0, -1).on("select", function () {
...
// we provided to it.
env.addEventListener("ready", function (evt) {
  var gazeTxt = text3D(0.25, "Gaze at house icon to return to selection").colored(0xffffff).addTo(instructions);

  var b = gazeTxt.geometry.boundingBox;

  gazeTxt.at(-b.max.x / 2, 0, -POSITIONING_RADIUS / 2);

  var back = box(b.max.x * 1.1, b.max.y * 1.3, 0.125).colored(0x225273, {
    side: THREE.BackSide,
    unshaded: true
  }).addTo(gazeTxt).at(b.max.x / 2, b.max.y / 2, 0);

  // Create the photospheres.
  var photos = ["test1", "test2", "test3", "test4", "test5", "test6", "test7"].map(function (img, i, surfaces) {
    return new Primrose.Controls.Image("images/" + img + ".jpg", {
      radius: 3,
      unshaded: true,
      progress: Preloader.thunk
    }).addTo(env.scene).latLon(0, i * 360 / surfaces.length, POSITIONING_RADIUS).on("select", function (evt) {
      return env.teleport(evt.hit.object.position);
    });
  });
